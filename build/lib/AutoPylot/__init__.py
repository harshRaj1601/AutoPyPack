...
class AutoPylot:
    """
    AutoPylot class for importing and using directly in Python files.
    Usage: import AutoPylot
    """
    @staticmethod
    ...
    def scan_file(file_path):
        ...
        print(f"[AutoPylot] Scanning file: {file_path}")
        mappings = load_mappings()
        
        try:
            imports = scan_imports(file_path)
            if not imports:
                print("[AutoPylot] No imports found in the file.")
                return
                
            print(f"[AutoPylot] Found {len(imports)} imports.")
            
            missing_packages = []
            for module_name in imports:
                # Skip standard library and internal modules
                if module_name in ('autopylot', 'AutoPylot') or module_name in sys.builtin_module_names:
                    continue
...
# Only auto-scan when directly imported by user code, not through the AutoPylot.py bridge
# The bridge file will handle scanning for users importing via "import AutoPylot"
if __name__ != "__main__":  # Not being run directly
    ...
    if caller_frame:
        ...
        # Only scan when directly imported by user code, not from AutoPylot.py or internal modules
        if (not caller_module.startswith('autopylot') and 
            not caller_module == '__main__' and 
            not 'cli' in caller_module and
            not caller_file.endswith('AutoPylot.py')):
            
            if os.path.exists(caller_file) and os.path.isfile(caller_file) and not caller_file.endswith('__init__.py'):
                AutoPylot.scan_file(caller_file)