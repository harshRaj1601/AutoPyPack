...
def load_mappings():
    ...
    try:
        ...
        print(f"[AutoPylot] Warning: Could not find mappings.json at {path}")
        return {}
...
def scan_imports(file_path):
    ...
    try:
        ...
        print(f"[AutoPylot] Error reading {file_path}: {str(e)}")
        return set()
        
    try:
        node = ast.parse(content, filename=file_path)
    except SyntaxError as e:
        print(f"[AutoPylot] Syntax error in {file_path}: {str(e)}")
        return set()
...
def main():
    ...
    if len(sys.argv) < 2:
        print("[AutoPylot] Error: No file specified.")
        print("[AutoPylot] Usage: python -m autopylot <file.py>")
        return
        
    script_path = sys.argv[1]
    if not os.path.exists(script_path):
        print(f"[AutoPylot] Error: File not found: {script_path}")
        return
        
    print(f"[AutoPylot] Scanning file: {script_path}")
    modules = scan_imports(script_path)
    
    if not modules:
        print("[AutoPylot] No imports found.")
        return
        
    print(f"[AutoPylot] Found {len(modules)} imports.")
